...
全てをGETリクエストだけで行えるように見えるため、
Webサーバとクライアントのプロトコルを、
データの更新も含めてGETで行いたくなることがあります。

しかし、これは一般的には良い実装とは言えません。
GETでサーバデータの更新をさせるのは、
HTTPの仕様で適切でないとされているからです。
