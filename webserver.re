...
「Webサーバ」とは何かを知った後、
Hello Worldサーバを実際に作成してみましょう。

本章全体で、
第13章「ネットワークプログラミング」で学んだことを前提にしています。
...
堅苦しく言えば、サーバは「サービスを要求する側に、そのサービスを提供」します。
「サービスを要求する側」を「クライアント(Client)」と呼びます。
クライアントという言葉自体は、
例えば日本でも、コンサルティングを受けるお客さんに対しても使われます。

特にWebサーバの場合、そのサービスというのは何らかの資源、
あるいは「リソース(Resource)」をクライアントに渡したり、
クライアントの持つリソースを受け取って保存したりします。

ジュース「サーバ」の場合、「クライアント」が要求する「リソース」はジュースでした。
Web「サーバ」の場合、「クライアント」であるWebブラウザやAndroidアプリが要求する「リソース」はWebページやその中の「お天気情報」や今回扱う「講義情報」です。
...
なお、本章では説明を簡略化するため、
「インターネット」と「クラウド」と「Web」の区別や、
「Webサーバ」と「インターネット上の他のサーバ」の区別、
「Webサーバ」と「Webアプリ」の区別については扱いません。
みなさんがWebブラウザで見る向こう側を全て
「Webサーバ」「Webアプリ」という一括りで考えます。
...
これらは提供するサービスの性質に応じて
「SaaS」「PaaS」「IaaS」「オンプレミス」などと
分類分けされることがあります。
大まかに言うと、誰が物理的なコンピュータを管理して、
OSそのものの機能をどこまで使うか、が異なります。
...
GAEではwebapp2ライブラリのRequestHandlerを継承することで、
Webサーバを実際に実装することが出来ます。
ここではMainHandlerというクラスがRequestHandlerを継承しています。
MainHandlerの実装ではget()関数をオーバーライドし、
実際にHTTPのGETリクエストを処理するための処理を記述しています。

HTTP GETリクエストは、実は第14章「ネットワーク」で既に登場していますが、
今回はクライアント側からリクエストを送るのではなく、
サーバ側でリクエストを処理する側です。
HTTPにまつわるほとんどの処理は、
GAEとその上に用意されたwebapp2のようなライブラリがやってくれるため、
実質たった数行でサーバが完成するのです。
...
「全てのパスをこのMainHandlerクラスが受け持つ」ように実装を変更してみましょう。
...
現在はブラウザ自体に豊富な機能があるため、
Webサーバとの通信の様子を調べる場合にはまずそれを使うのが良いでしょう。

例えばChromeには「デベロッパーツール」と呼ばれる機能があります。
デベロッパーツールを起動するには、
...
「Chrome デベロッパーツール」などと検索をすると多くの情報が得られるはずですので、
デベロッパーツールについて詳細に学びたい方は調べてみましょう。

==== Putty

第14章「ネットワークプログラミング」ですでに登場したPuttyのように、
さらに細かいレベルでWebサーバが返答する内容を調査したいことがあります。

Puttyがインストール済みであれば、実際にHello Worldサーバへ接続し、
どのようなHTTPレスポンスが返されるか、調べてみましょう。
そのレスポンスのうち、今回自分の実装が影響している行と
そうでない行はどれでしょうか。
...
//lead{
本節では、前節で実装したWebサーバを拡張し、
第10章で作った「シラバスアプリ」と連携出来るようにします。
//}
...
学習しました。
本節ではこのサーバを、
第10章で搭乗した「シラバスアプリ」と連携出来るように拡張します。

Webブラウザから講義データをアップロードする機能も追加しながら、
Webサーバと通信する上でAndroidアプリ開発者が気をつけるべきことを学びましょう。

=== 「シラバスサーバ」を考える

第10章「ユーティリティによる実践」でAndroidアプリを作成した際、
...
でした。
(@<img>{syllabus-screenshot})

//image[syllabus-screenshot][シラバスアプリ]{
//}

このURLは、講師がDropbox@<fn>{about_dropbox}というWebサービスを用いて
公開している、
静的な(アクセスの度に中身が変わったりしない)ファイルの場所を表しています。
もし講義内容を変更したいとしたら、
直接そのファイル自身を修正する必要があります。

//footnote[about_dropbox][@<href>{https://www.dropbox.com/}]

本節では、この「シラバス」データを管理して公開する
「シラバスサーバ」を実装することを考えましょう。
具体的には、「Hello Worldサーバ」を拡張することで以下の機能を実現します。

 * JSON形式とHTML形式によって講義データを配布出来るようにする。
 * Webサーバへブラウザから講義データを追加出来るようにする。
 * 最低限のセキュリティを確保する。

サーバ側を実装する前にまず確認したいのは、
「どのような方法でAndroidアプリとWebサーバが通信するか」です。
このルールがアプリとサーバ側で違っていると、通信は出来ません。

どのような通信を行えば良いかを知るため、
まず、上記のJSONデータをWebブラウザから眺めてみましょう。(@<img>{syllabus_data})


//image[syllabus_data][シラバスデータをWebブラウザで見た結果]{
//}

元のデータは人が読めるようにフォーマットされていないため大変読みづらいですが、
本質的には@<list>{json_data}で示すような構造をしています。

//list[json_data][JSONフォーマットを整形し、構造が分かりやすい部分だけ抜粋して掲載している]{
{
  "course": [
    ...
    {
...
Eclipse上でシラバスアプリの実装でListViewやIntentにおいて使われていた
データ構造と比較してみてください。
結果として、JSONで指定されたこれらのデータの中から、
...
この時点では、まだ保存するデータ構造を定義しているだけで、
そのデータを表示するためには使っていません。
さっそくJSONデータの一部として使ってみましょう。
...
なおGAEの本番サーバの仕様により、
本番サーバへindex.yamlを最初にアップロードした後しばらくの間は、
...
この場合はPythonプログラムで指定しているソートの方法とindex.yamlで生成しているインデックスが一致していないという意味で、
つまり本番サーバにアップロードしたプログラムか、
index.yamlに何か間違いがあります。

空の講義データでは意味がないので講義データの作成画面が欲しいところですが、
その前に一旦、HTMLで講義データを表示する実装もしておきましょう。

URLのパスの後に「?」をつけると、その後に
「クエリ文字列」(Query String)という追加のデータを送ることができます。
例えば(2014年9月現在)「@<href>{https://www.google.co.jp/search?q=html}」
というURLをWebブラウザに入力すると
「HTML」をGoogle検索することができますが、
このとき「q=html」がクエリ文字列です。
クエリ文字列は「=」と「&」を駆使することで
JavaやPythonのマップのようなデータ構造を持つことができます。
Google検索の場合は「q」というキーが「html」という値を持っている
と考えることが出来るでしょう。

このクエリ文字列の仕組みを用いて、
シラバスサーバのトップページの挙動を以下のように変更します。

 * クエリ文字列に指定がなければHTMLを返す (例 @<href>{http://localhost:8080/})
 * 「output=json」 というクエリ文字列をつけたらJSONを返す (例 @<href>{http://localhost:8080/?output=json})

GAEでは「self.request.get('output')」といった表現で、
クエリ文字列に与えられたキーと値の組み合わせを簡単に取得できます。

この表現とif分岐と組み合わせてトップページでのHTML表示とJSON表示を
分岐して表示する結果を@<list>{html_and_json_with_no_result}に示します。
なお、既に実装したJSON側の各行のインデントが1段下がっている点にも
注意してください。
Pythonではインデントがブロックの範囲を表します。

//list[html_and_json_with_no_result][HTMLページ形式も表示できるようにする]{
# -*- coding: utf-8 -*-
from google.appengine.ext import ndb
import webapp2
...
なお、この「仕様変更」によって、講義表のJSON形式データを受け取るURLが変わりました。
これ以降、Androidアプリ側で結果を取得するURLの末尾には
「?output=json」をつけておく必要があります。
(@<list>{changed_url})

//list[changed_url][HTMLに関わる変更で、シラバスアプリのURLの末尾に「?output=json」が必要となる。]{
...
講義データを作成するHTMLフォームを作り、シラバスサーバを完成させましょう。
ここで行うべきことが大きく分けて2つあります。

 * フォーム画面そのものを作ります。
 * フォーム画面から送信されたデータを受け取り、サーバに登録する部分を作ります。

まずフォーム画面そのものを作ります。
単純化のため、今はHTMLをPythonの複数行文字列としてプログラムに埋め込みます。
...
データを追加するフォーム画面を表示するため「CreateCourse」という
クラスを新たに追加しました。
このクラスは@<href>{http://localhost:8080/create}
に対応するURLへユーザがアクセスした時に、所定のHTMLを表示します。
「/create」というパスとCreateCourseの対応は、webapp2.WSGIApplication()
が受け取るリストに記述されています。

今回の実装では、HTTPのGETリクエストではなくPOSTリクエストを用いて、
Webサーバにデータを送信するようにHTMLフォームに指定されています
(「method="post"」の部分を参照)。
通常のWebサーバでは、ユーザがデータを閲覧する際にはGETリクエストのみ、
ユーザがデータを追加・更新する際にはPOSTリクエストのみが用いられるよう、
実装していれば大丈夫です。
なぜGETリクエストではなくPOSTリクエストを使うかの詳細は、
コラムを参照してください。

フォームがブラウザに表示されるようになりましたが、
そのフォームから送られたデータを受け取るための実装はまだ作っていません。
このままでは「405 Method Not Allowed」というエラーが出ます。

//image[method_not_allowed][POSTの受け口を作っていない]{
//}

というわけで、CreateCourseクラスにさらに受ける側を作ります。
POSTリクエストを受け取るため、get()関数ではなくpost()関数を
新たに追加します。(@<list>{implementing_post})

//list[implementing_post][CreateCourseの全実装]{
class CreateCourse(webapp2.RequestHandler):
    def get(self):
        self.response.headers['Content-Type'] = 'text/html; charset=UTF-8'
        self.response.write(form_html)
...
ここまでで、最初に提示した3つシナリオをすべて実装し終えました。
以下を確認してみて、実装に誤りがないことを確認してください。
...
以上で講義情報を追加できるだけの「シラバスサービス」が出来ました。

実際に使うのであれば編集と削除機能が必要なところですが、
これについては演習課題としましょう。
まだこのままでは機能の他に気がかりな点がいくつかあります。


===[column] なぜ更新はPOSTリクエストで行うのか

クエリ文字列を用いればGETリクエストだけで更新も行えるように見えるため、
実際そのような実装にしてしまいたくなることがあります。

しかし、これは一般的には良い実装とは考えられていません。
GETでサーバデータの更新をさせるのは、HTTPの仕様で適切でないとされています。
それに伴って、発見と修正が非常に面倒なバグに悩まされることすらありえます。

HTTPの仕様はRFC(Request For Comments)という
公開されたドキュメント群の一部として公開されています。
RFCのドキュメントにはそれぞれに番号がついているのですが、
その中でRFC 7231@<fn>{about_rfc7231}と呼ばれるドキュメントに、
GETとPOST、そしてその他のリクエストメソッドの仕様が決められています。

//footnote[about_rfc7231][@<href>{http://tools.ietf.org/html/rfc7231#section-4}]

RFC 7231では、要約すると
「GETリクエストはサーバの状態を変更せず(安全)、
何度行なっても結果が同じである(べき等)、
そしてその結果はキャッシュ可能であるべきである」
と定められています。
言い方を変えると、
GETリクエストでサーバのデータを書き換えるのは少なくとも行儀の良い動作ではない、
ということです。

例えば、インターネット上で通信する際、
GETリクエストのこの性質を上手く用いて結果をキャッシュする
コンピュータやソフトが途中経路に存在しても、文句を言うことは出来ません。
結果をキャッシュされると、
Webサーバに期待した更新リクエスト自体が送られてこないという事態が発生します。
POSTではこのような挙動は仕様上許されませんので、
その不適切な挙動を行うコンピュータについて少なくとも文句を言うことが出来ます。

GETリクエストではURLにデータが埋め込まれます。
そのため、URLの最大長の制約を受けます。
またWebブラウザのキャッシュにデータそのものが残ってしまいます。

POSTリクエストはもともとデータを送るためのものなので、
GETリクエストに関するこのような制約はありません。
ただし、データ送信以外にPOSTを用いるのは不適切ですので、
GETとPOSTを使い分けるのが適切、という結論になります。
...
知っている方が、
サーバとのやりとりを実装する上での落とし穴にははまりにくくなります。

「Android Open Textbook Project」
(@<href>{https://github.com/TechBooster/AndroidOpenTextbook})
の「Webサーバ」の章に、
本章学習後に学ぶべき内容が含まれていますので、
興味がある人は参考にしてください。
最新版のPDF版やHTML版は、
...
明らかに不足している講義表の「編集」「削除」機能を実装してみましょう。
また、講義作成画面も、テンプレートを使って見栄えを整えてみましょう。

=== 演習2: Androidアプリからの講義データの管理 (難)

Googleのような大手サービスではしばしば、
WebブラウザとAndroidアプリの両方でサーバ上の
リソースを操作することが出来ます。
今回のシラバスサーバでも同じことは出来ないでしょうか。
...
この演習についてはまず、
評価の高い既存のサービスがどのように
このシナリオを達成しているかを観察した方が、
Androidアプリ開発者としては得るものは多いでしょう。
