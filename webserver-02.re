...
//lead{
これまでは事前に容易されたデータを
Androidアプリから取得するだけでしたが、
本章ではそのデータを提供する側であるWebサーバの実装を通して、
Androidアプリの向こう側の世界について学びます。
//}
...
本章では、特に第13章「ネットワークプログラミング」で学んだ概念を前提としています。
...
=== Androidアプリでサーバが必要な場面

=== Androidアプリから使う場合の選択肢について

Androidアプリがサーバと通信する場合、
いくつかの選択肢があります。

まずおそらくもっとも多くなるのは、サーバはサービスプロバイダから
提供され、ユーザは単にそのWeb上のAPIを使うというケースです。
...
本章ではこれと真逆の「サーバとアプリをセットで作る」
ケースを想定しています。
つまり、期待されているのはサービス全体の設計で、
Androidアプリ単体のことだけでなく、
サーバの事情もある程度理解する必要があるということです。
こういったケースは目新しいことではなく、
特にもし、新しいサービスを世界に展開する場合にはしばしば必要になります。
昨日食事をご一緒したある別のAndroid開発者の方は
Androidアプリを開発しつつ、すでにあるWebサービス実装を
Androidアプリと緊密に連携させるため、アプリと
Web側のPHPプログラミングの両方を担当されているとのことでした。
...
これらは提供するサービスの性質に応じて「IaaS」「PaaS」「SaaS」など、
ざっくり言えば「どのくらいOSそのものの機能に近いか」で
分類分けされることが一般的です。
上記の一覧はその分類を一切せず混ぜてしまっているので、
知っている人にとっては気持ちが悪いものになっています。
また、厳密に分けられるとも限りません。

本節はサーバを含めた「ITインフラ」の講習ではないので、
この辺りの事情の説明はこれ以上行いません。

一つ言えるのは今回採用するGAEが唯一の選択肢でもなければ、
常に最良の選択肢であるとも言えないということです。

===[column] GAE Pythonを採用した理由
...
「ユーティリティによる実践」でシラバスアプリを実装しました。
このとき、AndroidアプリはWebサーバに保存された
JSONデータを、Volleyというライブラリで非同期にダウンロードし、
...
仮に「シラバスサービス」を実装するとしましょう。
Webサーバのデータが誰かから渡されて、
しかもオンラインで修正できないという状況では不十分です。
ブラウザからシラバスの内容を追加・編集出来る方が、
管理上の都合は良いはずです。

本節ではこのシラバスアプリで利用できるJSONデータを
Androidアプリに返すWebアプリケーションを作ってみます。
Androidアプリ側はURLを変更する以外は行いませんが、
本章の内容を踏まえればAndroidアプリとWebサーバの両方を
セットで作るための「勘」が鍛えられるでしょう。
...
Googleの本番サーバにアップロードし、世界にアプリケーションを公開しましょう。
